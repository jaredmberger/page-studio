(() => {
  ...
  window.addEventListener('message', event => {
    ...
    const ordinal = sameTag.indexOf(target);
    if (ordinal < 0) return;

    const ranges = sourceRangesForTag(source, tag);
    const match = ranges[ordinal];
    if (!match) return;
  ...
  function sourceRangesForTag(source, tag) {
    ...
    let hit;
    while ((hit = openTag.exec(source))) {
      if (/^<\//.test(hit[0])) continue;
      results.push(expandElementRange(source, hit.index, tag));
    }
    return results;
  }
